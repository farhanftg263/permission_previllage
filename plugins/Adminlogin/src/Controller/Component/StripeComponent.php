...
require_once (ROOT . '/vendor/stripePaymentApi.php');
...
class StripeComponent extends Component {
     ...
     * @var string
     */
    public $key = 'sk_test_9t1AAXpBYqXt4sOfdleBF3k4';
    ...
    /**
     * addNewCardToExistingCustomer method
     ...
     */
    public function addNewCardToExistingCustomer($customerId = null, $tokenId = null) {
        if (!$customerId) {
            throw new Exception(__('Customer Id is not provided'));
        }

        if (!$tokenId) {
            throw new Exception(__('Token Id is not provided'));
        }
    ...
    public function createBankAccount(array $data) {
        if (!$data['account_number'] || !$data['account_number']) {
            throw new Exception(__('Bank Information is not provided'));
        }
     ...
     * delete an existing card for the given customer
     *
     ...
     * @link https://stripe.com/docs/api/php#update_card
     */
    public function deleteBankAccount(array $data) {
        if (!$data['account_number'] && !$data['bank_account_number']) {
            throw new Exception(__('Bank Information is not provided'));
        }
     ...
     * @link https://stripe.com/docs/api/php#update_card
     */
    public function bankaccountlists(array $data) {
        if (!$data['bank_account_number'] || !$data['bank_account_number']) {
            throw new Exception(__('Account is not provided'));
        }
    ...
    private function request($method = null, $data = null) {
        ...
        try {
            switch ($method) {
                ...
                case 'addNewCardToExistingCustomer':
                    $cu = Customer::retrieve($data['customer_id']);
                    $cardData = array('source' => $data['token_id']);
                    $card = $cu->sources->create($cardData);
                ...
                case 'createBankAccount':
                    ...
                    $success = $this->fetch($account);

                    break;
                
                case 'deleteBankAccount':
                    
                    $account = \Stripe\Account::retrieve($data['account_number']);
                    
                    $bank_account = $account->external_accounts->retrieve($data['bank_account_number'])->delete();
                    
                    $success = $this->fetch($bank_account);
                    
                    break;
                
                case 'bankaccountlists':
                    
                    $account = \Stripe\Account::retrieve($data['bank_account_number']);
