...
<div class="box-header">
    <div class="col-sm-6" style="width: 20%;">
        <div id="example1_length" class="dataTables_length" style="float:left !important;">
            ...
            echo $this->Html->link('+ Add',
                ['controller' => 'CmsPages', 'action' => 'add'],
                ['title' => 'Add CMS Page','class'=>'btn btn-primary']
            );
            ?>                      
        </div>
    </div>
    <div class="col-sm-6" style="width: 80%;">
        ...
        echo $this->Form->create(
            null, 
            [
                'url' => ['controller' => 'CmsPages', 'action' => 'index'],
                'type' => 'get',
                'name'  => 'searchForm'
            ]
        );
            ...
            <span style="padding: 2px;float:right;margin-right:45px;">
                <?php 
                    echo $this->Html->link('Reset',
                        ['controller' => 'Cms-pages', 'action' => 'index'],
...
<div class="box-body">
    <table id="example1" class="table table-bordered table-striped">
        <?php  if(empty($cmsPages) || count($cmsPages)<=0){?>
        <tbody>
            <tr>
        ...
        <thead>
            <tr>
                <th width="5%">S.N.</th>
                <th width="20%"><?= $this->Paginator->sort('page_name', ' Page Name ',array('escape' => false));?></th>
                <th width="15%"><?= $this->Paginator->sort('page_title', ' Meta Title ',array('escape' => false));?></th>
                <th width="15%"><?= $this->Paginator->sort('meta_keyword', 'Meta Keyword ',array('escape' => false));?></th>
                <th width="20%"><?= $this->Paginator->sort('meta_description', 'Meta Description ',array('escape' => false));?></th>
                <th width="15%"><?= $this->Paginator->sort('created_on', 'Created ',array('escape' => false));?></th>
                <th width="10%"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php $i=0;foreach($cmsPages as $page):$i++?>
            <tr id="detele_<?= $page->id;?>">
                <td><?= $i;?></td>
                <td><?= $this->Html->link($page->page_name, ['action'=>'view', $page->id],['class'=>"various",'data-fancybox-type'=>"iframe",'escape' => false] );?></td>
                <td><?= h($page->page_title);?></td>
                <td><?= h($page->meta_keyword);?></td>
                <td><?= h($page->meta_description);?></td>
                <td><?= date("jS M, Y",strtotime($page->created_on));?></td>
                <td>
                    <?php
                    if($page->status == 1) { 
                    ...
                    echo $this->Html->link('<i class="fa fa-fw fa-edit"></i>',
                        ['controller' => 'CmsPages', 'action' => 'edit', $page->id],
                        [                            
                            'title' => 'Edit',
                            'escape' => false
                        ]
...
$(document).ready(function(){
        ...
        function(isConfirm){
            if (isConfirm) {
                $.ajax({
                    type: "POST",
                    data: {id: id},
                    url: '<?= $this->Url->build(['controller' => 'CmsPages', 'action' => 'delete'], true);?>',
                    dataType: 'json',
        ...
        function(isConfirm){
            if (isConfirm) {
                $.ajax({
                    ...
                    url: '<?= $this->Url->build(['controller' => 'CmsPages', 'action' => 'status'], true);?>',
                    dataType: 'json',
                    success: function(json) {           
                        if(json.status==1){
    ...
    $("#sk").autocomplete({
        source: '<?= $this->Url->build(['controller' => 'CmsPages', 'action' => 'searchPageName'], true);?>'
    });
});
</script>